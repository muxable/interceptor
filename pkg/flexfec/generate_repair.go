...
func NewRepairPacketFlex(seqnum uint16, fecheader FecHeaderFlexibleMask, repairPayload []byte) rtp.Packet {
...
func NewRepairPacketLD(seqnum uint16, fecheader FecHeaderLD, repairPayload []byte) rtp.Packet {
	repairPacket := rtp.Packet{
		Header: rtp.Header{
			Version:        2,
			Padding:        false,
...
func GenerateRepairFlex(srcBlockBitstrs *[][]byte, mask uint16, optionalMask1 uint32, optionalMask2 uint64, SN_base uint16) []rtp.Packet {
	...
	fecBitstring := ToFecBitString(coveredBitstrings)

	fecheader, repairPayload := ToFecHeaderFlexibleMask(fecBitstring, SN_base, mask, optionalMask1, optionalMask2)
...
func GenerateRepairRowFec(srcBlkBitstrs *[][]byte, L int, is2D bool, SN_Base uint16) []rtp.Packet {
	...
	for i := 0; i < len(*srcBlkBitstrs); i += L {
		...
		fecBitString := ToFecBitString(rowBitstrings)
		fecheader, repairPayload := ToFecHeaderLD(fecBitString, SN_Base+uint16(i), uint8(L), uint8(0))
...
func GenerateRepairColFec(srcBlkBitstrs *[][]byte, L, D int, SN_Base uint16) []rtp.Packet {
	...
	for j := 0; j < L; j++ {
		...
		fecBitString := ToFecBitString(colbitstrings)
		fecheader, repairPayload := ToFecHeaderLD(fecBitString, SN_Base+uint16(j), uint8(L), uint8(D))
